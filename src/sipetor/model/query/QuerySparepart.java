...
public class QuerySparepart implements InterfaceSparepart {
    ...
    public boolean update(Sparepart s) {
        String sql = "UPDATE sparepart SET nama_sparepart=?, harga_sparepart=?, stock_sparepart=? WHERE id_sparepart=?";
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
    ...
    public Sparepart getOneSparepartByID(int id_sparepart) {
        ...
        try{
            if (SQLConnection.getConnection()==null){
                ...
                ResultSet rs = statement.executeQuery();
                while (rs.next()){
                    output = new Sparepart(rs.getByte(1), rs.getString(2), rs.getByte(3), rs.getByte(4));        
            }
            statement.close();
            }
